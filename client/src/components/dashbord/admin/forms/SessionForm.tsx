...
const SessionForm = (
    {setOpen}: SessionFormProps
) => {

    const [createBookingSession, {isLoading}] = useCreateBookingSessionMutation();
  return (
                  ...
                  {({ values, isSubmitting }) => (
                    <Form className='flex w-full flex-col gap-10 text-white sm:text-black'>
                      ...
                      <div className="flex justify-end gap-3">
                          <button type="button" className="bg-black px-3.5 py-2.5 text-center text-sm font-semibold text-white shadow-xs hover:bg-black/70 focus-visible:outline-2 focus-visible:outline-offset-2 rounded-md focus-visible:outline-black flex gap-2" onClick={()=>setOpen(false)}>
                              <span> x </span><span>Close</span>
                          </button>
                          <button
