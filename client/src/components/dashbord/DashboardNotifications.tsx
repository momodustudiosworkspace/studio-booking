    ...
    return (
        <div className="flex flex-col gap-3">
            {NOTIFICATIONS.map((notification) => {
                return <BookingCardNotification key={notification.id} message={notification.message} date={notification.date} type={notification.type} id={notification.id} body={notification?.body} />
           })}
        </div>
    );
};
